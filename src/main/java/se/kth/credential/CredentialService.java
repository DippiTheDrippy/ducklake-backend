...
import se.kth.common.PostgresException;
...
public class CredentialService {
    ...
    public Credential getDatasetCredential(String datasetId, String email) {
        ...
        return credentialRepository.findByDataset(UUID.fromString(datasetId)).orElse(null);
    }

    public List<Credential> listUserCredentials(String email) {
        User user = userRepository.findByEmail(email)
                .orElseThrow(() -> new NoSuchElementException("No such user!"));

        return credentialRepository.listByUser(user.getId());
    }
    ...
    public CreateCredentialResponse createCredential(String datasetId, String email, CreateCredentialRequest req) {
        ...
        if (permissionRepository.hasAccessLevel(user.getId(), d.getId(), req.access())) {
            ...
            CreateKeyResponse key = garageRepository.createKey(keyName, req.expiresAt(), req.neverExpires());
            ...
            if (req.access().compareTo(AccessLevel.READ) == 0) {
                dbCred = postgresUserRepository.createReadOnlyUser(d.getName(), req.expiresAt(), req.neverExpires());
                garageRepository.allowKey(key.accessKeyId(), bucket.id(), false);
            } else if (req.access().compareTo(AccessLevel.WRITE) == 0) {
                dbCred = postgresUserRepository.createReadWriteUser(d.getName(), req.expiresAt(), req.neverExpires());
    ...
    public CreateCredentialResponse rotateCredential(String id, String email) {
        ...
        String password = postgresUserRepository.rotateUserPassword(cred.getPostgresUsername(), cred.getExpiresAt(),
        ...
        String keyName = "ducklake_cbh_" + UUID.randomUUID().toString();
        CreateBucketResponse bucket = garageRepository.getBucketByGlobalAlias(d.getBucketName());
        CreateKeyResponse key = garageRepository.createKey(keyName, cred.getExpiresAt(), cred.getExpiresAt() == null);
